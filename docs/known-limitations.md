...
macOS release-build support is merged, and its real-device validation baseline was completed. The v0.4.9 artifact remains unsigned/ad-hoc until Apple Developer ID signing and notarization are configured. First launch may require Control-click > Open or Privacy & Security approval after the user confirms the file came from the project GitHub Release.
...
- rpm, AppImage, Snap, and Flatpak packages are not in scope for v0.4.9.
...
User-triggered batch Test and Update use a fixed maximum of six concurrent hosts. The limit is not configurable, per-host results are independent rather than transactional across the batch, and install/uninstall remain single-host actions. Batch Update may require one combined process-impact dialog after all preflights finish. Hosts with no impact continue automatically; selected impacted hosts are revalidated and receive individual `SIGTERM`; unselected, changed, newly impacted, or unverifiable hosts fail before PATH repair and require the user to exit related processes before retrying. Hiding a progress dialog keeps the backend task running; detailed command output is retained under collapsed step cards in Tasks rather than streamed into the main UI by default.
