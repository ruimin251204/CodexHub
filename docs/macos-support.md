...
Status: macOS release-build support is merged into `master`, and the real-device validation baseline was completed. The v0.5.2 macOS public artifact remains unsigned/ad-hoc.
...
- v0.5.2 remains unsigned/ad-hoc, so Gatekeeper approval may still be required on first launch.
