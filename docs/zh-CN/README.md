...
- 单主机和批量 Update 都会先检查引用托管 standalone release 的当前用户进程。CodexHub 会在不返回原始 argv 的前提下区分 Codex App 后台服务、SSH 代理和 CLI/会话，再要求明确确认。仅对已批准且身份复核一致的进程逐个发送 `SIGTERM`；无法分类、未选择、身份变化、新增影响或无法复核的主机在 PATH 修复和安装前失败并保持不变。
