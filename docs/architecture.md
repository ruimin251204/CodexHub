...
2. `process-impact`: for every Update, revalidate the complete previewed current-UID PID/starttime/release/comm/process-kind set, reject unknown, new, or changed identities, and send individual `SIGTERM` signals only to the approved host's exact PIDs. Install and uninstall keep the single-host bypass because they do not replace an existing managed runtime through this Update path.
