...
2. `process-impact`: for batch Update, revalidate the complete previewed current-UID PID/starttime/release/comm set, reject new or changed identities, and send individual `SIGTERM` signals only to the approved host's exact PIDs; single-host maintenance records this gate as skipped.
