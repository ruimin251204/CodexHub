...
* Single-host and batch Update first inspect current-user processes that reference managed standalone releases. CodexHub classifies confirmed Codex App services, SSH proxies, and CLI/sessions without returning raw argv, then asks for explicit approval. Approved identities are revalidated and stopped with individual `SIGTERM` signals; unknown, unselected, changed, newly affected, or unverifiable hosts fail before PATH repair or installation and remain unchanged.
