...
* Batch Update first inspects all selected hosts for current-user processes that reference managed standalone releases. CodexHub then opens one combined confirmation with per-host PID, process name, and version details. Hosts selected in that dialog are revalidated and stopped with individual `SIGTERM` signals; unselected or unverifiable hosts fail before PATH repair or installation and remain unchanged.
